...
export default function SignIp() {
    ...
    const register = async () => {
        try {
            ...
            console.log(error);
            alert('Sign in failed: ' + error.message); // Reverted to original alert
        }
    };

    const makeAnAccount = async () => {
        ...
        <SafeAreaView className="flex-1 bg-gray-900 justify-center items-center p-6">
            ...
            <View className="w-full max-w-md bg-gray-800 p-8 rounded-2xl shadow-lg">
                ...
                <TouchableOpacity
                    ...
                    onPress={makeAnAccount}
                >
                    <Text className="text-white text-xl font-bold">Dont have have an account?</Text>
                </TouchableOpacity>
            </View>
        </SafeAreaView>
    );
